...
class TestPriorBoxOp(hu.HypothesisTestCase):
    ...
    def test_prior_box(
        ...
        if use_tensor_shapes:
            image = np.array([image_size, image_size], dtype=np.int64)
            fm = np.array([fm_size, fm_size], dtype=np.int64)
        else:
